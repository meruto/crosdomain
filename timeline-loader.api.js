    ...
    if (!json.data[i].body_html.match(/img.*src=/))
    {
      ...
      else if (json.data[i].body.match(/((http:|https:)\/\/www\.amazon\..*\/([a-zA-Z0-9_\-]+)\/.*)/))
      {
        ...
        if ('undefined' != (typeof id)){
          ...
          json.data[i].body_html = json.data[i].body.replace(/((http:|https:)\/\/[\x21-\x26\x28-\x7e]+)/gi, '<a href="$1"><div class="urlBlock"><img src="http://mozshot.nemui.org/shot/large?$1"><br />$1</div>$1</a>');
        }
      }
...
//ここかあら本文の抽出技術
	  ...
	  uri = ''
        ...
        $.get(uri, (data) => {
          $('#'
            + id2
            + '').prepend(data.responseText.match(/title>(.*)<\/title>/)[1] + ' ■<br>');
        });
		  
     
	  
        
		json.data[i].body_html = json.data[i].body.replace(/((http:|https:)\/\/[\x21-\x26\x28-\x7e]+)/gi, '<a href="$1"><div class="urlBlock">\
		<br><div id="'
            + id2
            + '"></div>\
<img src="http://www.google.com/s2/favicons?domain=$1"><br><img src="http://mozshot.nemui.org/shot/large?$1"><br />$1</div></a><br>'
            + id
            + '');
